...
\section{高速视频采集程序的设计}
	...
	对DigiRetina 16相机设备的操作通常分为以下几个步骤，其操作流程如图\ref{fig:flow_chart1}所示。
    ...
    \node (init) [process] {驱动初始化};
    \node (allocate) [process, right of=init,node distance=4cm] {相机初始化并分配内存空间};
    \node (open) [process, right of=allocate,node distance=4cm] {设置相机参数};
    \node (op) [process, below of=open,yshift=0.5cm] {开始捕获数据};
    \node (close) [process, below of=op,yshift=0.5cm] {确保拍摄已经完成，并获得数据};
	\node (free) [process, left of=close,node distance=4cm] {释放内存空间并关闭相机};
